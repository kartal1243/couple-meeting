...
const DmChat = memo(function DmChat({ authUser, dmConversations, dmActiveChat, setDmActiveChat, dmMessages, dmInput, setDmInput, sendDm, openDm, typingUsers, sendDmTyping, sendDmStopTyping, followUser, unfollowUser, isFollowingUser, styles }) {
  ...
  const handleSend = (e) => {
    ...
    sendDm(dmActiveChat.username, dmInput.trim());
    setDmInput('');
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
      ...
      <div className="cm-social-dm-header" style={{ padding: '10px 14px', borderBottom: '1px solid #25313a', display: 'flex', alignItems: 'center', gap: 10, background: '#111b21' }}>
        <button onClick={() => setDmActiveChat(null)} style={{ background: 'none', border: 'none', color: '#53e6bc', cursor: 'pointer', fontSize: 18, fontWeight: 900, padding: '4px 6px' }}>&larr;</button>
      ...
      {/* Input */}
      <form className="cm-social-dm-input" onSubmit={handleSend} style={{ padding: 10, borderTop: '1px solid #25313a', display: 'flex', gap: 7, background: '#111b21' }}>
        <input value={dmInput} onChange={handleInputChange} placeholder="Mesaj yaz..." style={{ flex: 1, background: '#1f2c34', border: '1px solid #2a3942', color: '#e9edef', padding: '9px 12px', borderRadius: 10, fontSize: 13, outline: 'none' }} />
