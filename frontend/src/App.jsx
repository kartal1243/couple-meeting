...
function App() {
  ...
  if (!inRoom) {
    return (
      <div style={{ backgroundColor: '#06080c', color: '#e0e6ed', minHeight: '100vh', width: '100vw', margin: 0, padding: 0, fontFamily: "'Inter', sans-serif" }}>
        <header style={{ padding: '20px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #1a202c', width: '100%', boxSizing: 'border-box' }}>
        ...
        <div style={{ width: '100%', maxWidth: '850px', margin: '40px auto 0 auto', textAlign: 'center', padding: '0 20px', boxSizing: 'border-box' }}>
          ...
          <div style={{ background: '#141a23', borderRadius: '20px', padding: '32px', border: '1px solid #2d3748', boxShadow: '0 25px 60px rgba(0,0,0,0.6)', maxWidth: '500px', margin: '0 auto', boxSizing: 'border-box' }}>
            ...
            {tab === 'create' && (
              <form onSubmit={handleCreateRoomSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <input 
                  ...
                  style={{ padding: '14px', borderRadius: '10px', border: '1px solid #2d3748', background: '#0b0e14', color: '#fff', fontSize: '14px', boxSizing: 'border-box' }}
                />
                <input 
                  type="password" 
  ...
  return (
    <div style={{ backgroundColor: '#06080c', color: '#e0e6ed', height: '100vh', width: '100vw', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
      ...
      <div className="room-layout" style={{ flex: 1, display: 'flex', width: '100vw', height: 'calc(100vh - 56px)', overflow: 'hidden', margin: 0, padding: 0 }}>
        ...
        <div className="video-stage" style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#000', position: 'relative', height: '100%' }}>
          
          <form onSubmit={handleMediaSubmit} style={{ padding: '10px 16px', background: '#0e121a', borderBottom: '1px solid #1a202c', display: 'flex', gap: '10px', flexShrink: 0, boxSizing: 'border-box', width: '100%' }}>
            <input 
              type="text" 
              placeholder="🎬 YouTube veya Video Linki Yapıştırıp Enter'a Basın..." 
              value={inputUrl}
              onChange={(e) => setInputUrl(e.target.value)}
              style={{ flex: 1, padding: '10px 14px', borderRadius: '8px', border: '1px solid #2d3748', background: '#06080c', color: '#fff', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }}
            />
          ...
          <div style={{ flex: 1, position: 'relative', width: '100%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', background: '#000' }}>
            {mediaType === 'none' && (
              <div style={{ textAlign: 'center', color: '#4a5568', padding: '20px' }}>
                ...
                <h3 style={{ color: '#a0aec0', margin: '0 0 8px 0', fontSize: '18px' }}>Sinema Ekranı Tam Ekran Olarak Hazır</h3>
                <p style={{ fontSize: '13px', margin: 0 }}>Yukarıdaki alandan izlemek istediğiniz video URL'sini yapıştırın.</p>
              </div>
            )}
          ...
          <div style={{ padding: '12px 20px', background: '#0e121a', borderTop: '1px solid #1a202c', display: 'flex', gap: '12px', alignItems: 'center', flexShrink: 0, boxSizing: 'border-box', width: '100%' }}>
        ...
        <div className="chat-sidebar" style={{ width: '320px', background: '#0e121a', borderLeft: '1px solid #1a202c', display: 'flex', flexDirection: 'column', flexShrink: 0, height: '100%' }}>
          
          <div style={{ padding: '14px 18px', borderBottom: '1px solid #1a202c', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxSizing: 'border-box' }}>
            <h3 style={{ margin: 0, fontSize: '14px', color: '#f5b041', fontWeight: 'bold' }}>💬 Canlı Sohbet</h3>
            <span style={{ fontSize: '11px', color: '#718096' }}>Profil: {myAvatar}</span>
          </div>

          <div style={{ flex: 1, overflowY: 'auto', padding: '14px', display: 'flex', flexDirection: 'column', gap: '12px', boxSizing: 'border-box' }}>
            {messages.length === 0 ? (
              ...
              messages.map((msg, idx) => (
                <div key={idx} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', textAlign: 'left' }}>
                  <div style={{ fontSize: '22px', background: '#1a202c', padding: '4px', borderRadius: '50%', border: '1px solid #2d3748', lineHeight: 1, flexShrink: 0 }}>
                    {msg.avatar || '🐱'}
                  </div>
          ...
          <form onSubmit={handleSendMessage} style={{ padding: '12px', borderTop: '1px solid #1a202c', display: 'flex', gap: '8px', background: '#0e121a', boxSizing: 'border-box', width: '100%' }}>
            <input 
              type="text" 
              placeholder="Mesaj yaz..." 
              value={chatInput} 
              onChange={(e) => setChatInput(e.target.value)} 
              style={{ flex: 1, padding: '10px 12px', borderRadius: '8px', border: '1px solid #2d3748', background: '#06080c', color: '#fff', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }} 
            />
