...
const THEMES = {
  default: { bg: 'linear-gradient(135deg, #090d16 0%, #05070c 100%)', cardBg: '#111b21', primary: '#f5b041' },
...
function App() {
  ...
  // =========================================================================
  // WATCH2GETHER TARZI PROFESYONEL ANA SAYFA (LANDING PAGE)
  // =========================================================================
  if (!inRoom) {
    return (
      <div style={{ ...styles.app, overflowY: 'auto' }}>
        <header style={{ padding: '20px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.08)', background: '#090d16' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }} onClick={() => setInRoom(false)}>
            <div style={{ width: '42px', height: '42px', borderRadius: '12px', background: 'linear-gradient(135deg, #ff4757, #f5b041)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px' }}>❤️⚡</div>
            <div>
              <h1 style={{ margin: 0, fontSize: '22px', color: '#fff', fontWeight: '900', letterSpacing: '-0.5px' }}>Couple Meeting</h1>
              <span style={{ fontSize: '11px', color: '#f5b041', fontWeight: 'bold' }}>Birlikte İzle & Dinle</span>
            </div>
          </div>
        ...
        {showProfileModal && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ ...styles.card, width: '420px', textAlign: 'center' }}>
              <h3 style={{ margin: '0 0 16px 0', color: currentTheme.primary, fontSize: '20px', fontWeight: '800' }}>👤 Profil Kartı</h3>
              <div style={{ marginBottom: '20px' }}>
                <label style={{ fontSize: '12px', color: '#8696a0', fontWeight: 'bold', display: 'block', marginBottom: '10px' }}>AVATAR SEÇ</label>
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', background: '#0b141a', padding: '12px', borderRadius: '14px', border: '1px solid #222d34' }}>
                  {AVATARS.map((emoji) => (
                    <span key={emoji} onClick={() => handleAvatarSelect(emoji)} style={{ fontSize: '26px', padding: '6px', borderRadius: '10px', cursor: 'pointer', background: myAvatar === emoji ? '#f5b041' : 'transparent' }}>
                      {emoji}
                    </span>
                  ))}
                </div>
              ...
              <div style={{ marginBottom: '24px', textAlign: 'left' }}>
                <label style={{ fontSize: '12px', color: '#8696a0', fontWeight: 'bold', display: 'block', marginBottom: '8px' }}>ŞEHİR</label>
        ...
        <section style={{ maxWidth: '900px', margin: '50px auto 40px auto', padding: '0 24px', textAlign: 'center' }}>
          <h2 style={{ fontSize: '52px', fontWeight: '900', color: '#fff', margin: '20px 0 10px 0', letterSpacing: '-1.5px', lineHeight: '1.2' }}>
            spend time <span style={{ color: '#f5b041', borderBottom: '3px solid #f5b041' }}>together</span>
          </h2>

          <p style={{ color: '#8696a0', fontSize: '16px', maxWidth: '600px', margin: '0 auto 40px auto', lineHeight: '1.6' }}>
            Sevgilinizle veya arkadaşlarınızla YouTube videolarını ve filmleri eş zamanlı izleyin.
          </p>

          {errorMessage && (
            <div style={{ background: '#ea0038', color: '#fff', padding: '12px 16px', borderRadius: '12px', fontWeight: 'bold', marginBottom: '20px', maxWidth: '480px', margin: '0 auto 20px auto', fontSize: '14px' }}>
              {errorMessage}
            </div>
          )}

          {/* WATCH2GETHER TARZI ORTA DEV BUTON VE KART */}
          <div style={{ ...styles.card, maxWidth: '480px', margin: '0 auto 50px auto', textAlign: 'center', border: '1px solid #f5b04144', boxShadow: '0 30px 60px rgba(0,0,0,0.8)' }}>
            
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', background: '#0b141a', padding: '6px', borderRadius: '12px', border: '1px solid #222d34' }}>
              <button onClick={() => setActiveTab('create')} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: 'none', background: activeTab === 'create' ? '#f5b041' : 'transparent', color: activeTab === 'create' ? '#131822' : '#8696a0', fontWeight: 'bold', cursor: 'pointer' }}>
                🚀 Oda Oluştur
              </button>
              <button onClick={() => setActiveTab('join')} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: 'none', background: activeTab === 'join' ? '#f5b041' : 'transparent', color: activeTab === 'join' ? '#131822' : '#8696a0', fontWeight: 'bold', cursor: 'pointer' }}>
                🚪 Odaya Katıl
              </button>
            </div>

            {activeTab === 'create' ? (
              <form onSubmit={handleCreateRoomSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px', textAlign: 'left' }}>
                <input type="text" placeholder="Oda İsmi Belirleyin" value={roomId} onChange={(e) => setRoomId(e.target.value)} style={{ ...styles.input, width: '100%', boxSizing: 'border-box' }} />
                <input type="password" placeholder="Şifre (İsteğe Bağlı)" value={roomPassword} onChange={(e) => setRoomPassword(e.target.value)} style={{ ...styles.input, width: '100%', boxSizing: 'border-box' }} />
               
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#0b141a', padding: '10px 14px', borderRadius: '10px', border: '1px solid #222d34' }}>
                ...
                <button type="submit" style={{ ...styles.buttonPrimary, width: '100%', marginTop: '8px' }}>Kendi Odanızı Oluşturun 🚀</button>
              </form>
            ) : (
              <form onSubmit={handleJoinRoomSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px', textAlign: 'left' }}>
                <input type="text" placeholder="Oda İsmi veya Linki Girin" value={joinRoomInput} onChange={(e) => setJoinRoomInput(e.target.value)} style={{ ...styles.input, width: '100%', boxSizing: 'border-box' }} />
                <input type="password" placeholder="Oda Şifresi (Varsa)" value={joinPassInput} onChange={(e) => setJoinPassInput(e.target.value)} style={{ ...styles.input, width: '100%', boxSizing: 'border-box' }} />
                <button type="submit" style={{ ...styles.buttonPrimary, width: '100%', background: '#3742fa', color: '#fff', marginTop: '8px' }}>Odaya Giriş Yap 🚪</button>
              </form>
            )}

            {recentRooms.length > 0 && (
              <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid #222d34', textAlign: 'left' }}>
                <span style={{ fontSize: '11px', color: '#8696a0', fontWeight: 'bold', display: 'block', marginBottom: '8px' }}>SON GİRDİĞİN ODALAR</span>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {recentRooms.map((rId) => (
                    <button key={rId} onClick={() => socket.emit('join_room', { roomId: rId, password: '', userId, userCity, username, avatar: myAvatar })} style={{ background: '#202c33', color: '#f5b041', border: '1px solid #f5b04133', padding: '6px 12px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px' }}>
                      🚪 {rId}
                    </button>
                  ))}
                </div>
          ...
          {/* HAKKINDA / ÖZELLİKLER */}
          <div style={{ textAlign: 'left', background: '#111b21', padding: '24px', borderRadius: '16px', border: '1px solid #222d34', marginBottom: '50px' }}>
            <h3 style={{ margin: '0 0 14px 0', fontSize: '18px', color: '#fff' }}>Couple Meeting Hakkında</h3>
            <p style={{ color: '#8696a0', fontSize: '14px', lineHeight: '1.6', marginBottom: '16px' }}>
              Couple Meeting ile YouTube videolarını, dizileri ve müzikleri sevdiklerinizle tamamen senkronize bir şekilde izleyebilirsiniz. Bir oda oluşturun, linki partnerinize gönderin ve eğlencenin tadını çıkarın[cite: 1, 8].
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '10px', fontSize: '13px', color: '#e9edef' }}>
              <div>✔ Video ve ses için eş zamanlı oynatıcı</div>
              <div>✔ Oda içinde anlıksız canlı sohbet[cite: 1, 8]</div>
              <div>✔ Şifreli ve güvenli özel odalar[cite: 3, 8]</div>
              <div>✔ Kolay oynatma listesi yönetimi[cite: 1, 8]</div>
            </div>
          </div>
    ...
    <div style={{ ...styles.app, display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ marginBottom: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <div>
                  ...
                  <select value={roomTheme} onChange={(e) => setRoomTheme(e.target.value)} style={{ ...styles.input, width: '100%', boxSizing: 'border-box', cursor: 'pointer' }}>
                    <option value="default" style={{ background: '#111b21' }}>Altın Sarısı (Varsayılan)</option>
      ...
      <header style={{ height: '60px', padding: '0 28px', background: currentTheme.cardBg, borderBottom: '1px solid #222d34', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, width: '100vw', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }} onClick={handleLeaveRoom}>
          ...
          <span style={{ fontSize: '11px', background: 'rgba(245, 176, 65, 0.15)', color: currentTheme.primary, padding: '4px 12px', borderRadius: '20px', fontWeight: 'bold', border: '1px solid rgba(245, 176, 65, 0.3)' }}>
            Kişi: {currentRoomInfo.userCount}/{currentRoomInfo.maxUsers}
          </span>
        </div>

        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <button onClick={handleCopyLink} style={{ background: copied ? '#2ed573' : currentTheme.primary, color: '#131822', border: 'none', padding: '8px 14px', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px' }}>
      ...
      <div style={{ flex: 1, display: 'flex', width: '100vw', height: 'calc(100vh - 60px)', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#000', position: 'relative' }}>
          ...
          <div style={{ padding: '12px 20px', background: currentTheme.cardBg, borderBottom: '1px solid #222d34', zIndex: 999, display: 'flex', gap: '10px', alignItems: 'center', position: 'relative' }}>
            ...
            <button onClick={() => handleOpenAddModal(null)} style={{ ...styles.buttonPrimary, background: '#2ed573', color: '#131822' }}>➕ Listeye Ekle</button>
              ...
              <div style={{ position: 'absolute', top: '62px', left: '20px', right: '20px', ...styles.card, padding: '14px', zIndex: 9999, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                {searchResults.map((song) => (
                  <div key={song.id} style={{ display: 'flex', alignItems: 'center', gap: '14px', background: '#111b21', padding: '8px 12px', borderRadius: '10px', border: '1px solid #222d34' }}>
                    ...
                    <button onClick={() => handleSelectSearchResult(song, false)} style={{ ...styles.buttonPrimary, padding: '6px 12px', fontSize: '12px', background: '#2ed573', color: '#131822' }}>+ Klasöre Ekle</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ padding: '14px 24px', background: currentTheme.cardBg, borderTop: '1px solid #222d34', display: 'flex', gap: '14px', alignItems: 'center' }}>
            <button onClick={handlePlay} style={{ ...styles.buttonPrimary, flex: 1, background: '#2ed573', color: '#131822' }}>▶ Ortak Oynat</button>
            <button onClick={handlePause} style={{ ...styles.buttonPrimary, flex: 1, background: '#ffa502', color: '#131822' }}>⏸ Ortak Durdur</button>
            <div style={{ display: 'flex', gap: '6px' }}>
              {['❤️', '🔥', '😂', '😮', '👏', '😍'].map((emoji) => (
          ...
          {sidebarTab === 'chat' ? (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#0b141a' }}>
              ...
              <form onSubmit={handleSendMessage} style={{ padding: '8px 10px', borderTop: '1px solid #222d34', display: 'flex', gap: '6px', background: currentTheme.cardBg }}>
                ...
                <button type="submit" style={{ ...styles.buttonPrimary, background: currentTheme.primary, borderRadius: '50%', width: '34px', height: '34px', padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px' }}>➤</button>
              </form>
            </div>
          ) : (
            <div style={{ flex: 1, padding: '14px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px', background: '#0b141a' }}>
              <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', paddingBottom: '4px' }}>
                {categories.map((cat) => (
                  <button
                    ...
                    style={{ background: selectedCategory === cat ? currentTheme.primary : '#111b21', color: selectedCategory === cat ? '#131822' : '#8696a0', border: '1px solid #222d34', padding: '5px 10px', borderRadius: '16px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', whiteSpace: 'nowrap' }}
                  >
                    📁 {cat}
                  </button>
                ))}
              ...
              <form onSubmit={handleCreateCategory} style={{ display: 'flex', gap: '6px' }}>
                ...
                <button type="submit" style={{ ...styles.buttonPrimary, background: currentTheme.primary, padding: '6px 10px', fontSize: '11px' }}>Aç</button>
              </form>

              <div style={{ display: 'flex', gap: '4px', background: '#111b21', padding: '3px', borderRadius: '10px', border: '1px solid #222d34' }}>
                <button onClick={() => handleModeChange('sequence')} style={{ flex: 1, padding: '5px', borderRadius: '6px', border: 'none', background: playMode === 'sequence' ? currentTheme.primary : 'transparent', color: playMode === 'sequence' ? '#131822' : '#8696a0', fontWeight: 'bold', cursor: 'pointer', fontSize: '10px' }}>▶ Sırayla</button>
                <button onClick={() => handleModeChange('shuffle')} style={{ flex: 1, padding: '5px', borderRadius: '6px', border: 'none', background: playMode === 'shuffle' ? currentTheme.primary : 'transparent', color: playMode === 'shuffle' ? '#131822' : '#8696a0', fontWeight: 'bold', cursor: 'pointer', fontSize: '10px' }}>🔀 Rastgele</button>
                <button onClick={() => handleModeChange('alphabetical')} style={{ flex: 1, padding: '5px', borderRadius: '6px', border: 'none', background: playMode === 'alphabetical' ? currentTheme.primary : 'transparent', color: playMode === 'alphabetical' ? '#131822' : '#8696a0', fontWeight: 'bold', cursor: 'pointer', fontSize: '10px' }}>🔤 A-Z</button>
              </div>
              ...
              {getFilteredPlaylist().length === 0 ? (
                ...
                getFilteredPlaylist().map((item) => (
                  <div key={item.id} onClick={() => handleSelectPlaylistItem(item)} style={{ background: mediaSrc === item.src ? 'rgba(245, 176, 65, 0.15)' : '#111b21', border: mediaSrc === item.src ? `1px solid ${currentTheme.primary}` : '1px solid #222d34', padding: '10px', borderRadius: '10px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
