          ...
          {publicRooms.slice(0, 9).map((r, i) => (
            <div className="cm-room" key={r.id} onClick={() => onJoinRoom(r)} style={{ animationDelay: `${i * 0.08}s` }}>
              ...
              <div className="cm-room-top">
                <div className="cm-room-icon-wrap" style={{ background: r.roomType === 'music' ? 'linear-gradient(135deg,rgba(168,85,247,.2),rgba(236,72,153,.15))' : 'linear-gradient(135deg,rgba(37,99,235,.2),rgba(59,130,246,.15))' }}>
                  <span className="cm-room-emoji">{r.roomType === 'music' ? '🎵' : '🎬'}</span>
                </div>
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  <span style={{ color:'#64748b', fontSize:9, fontWeight:800, padding:'2px 8px', borderRadius:8, background: r.userCount > 0 ? 'rgba(34,197,94,.12)' : 'rgba(255,255,255,.05)', color: r.userCount > 0 ? '#22c55e' : '#64748b' }}>
              ...
              <div className="cm-room-meta">
                {r.roomType === 'music' ? 'Müzik odası' : 'Video odası'}
                {r.isVip ? ' • 👑 VIP' : ''}
              </div>
