          ...
          <div style={{ display: 'flex', gap: 7, alignItems: 'center' }}>
            {authUser ? (
              <button
                ...
                style={{ background: '#202c33', color: '#fff', border: '1px solid #2c3b44', padding: '8px 10px', borderRadius: 10, fontWeight: 800 }}
              >
                Çıkış
              </button>
            ) : (
              <button
                ...
                style={{ background: '#00a884', color: '#fff', border: 'none', padding: '8px 10px', borderRadius: 10, fontWeight: 900 }}
              >
                Giriş
              </button>
            )}
            <button
              ...
              style={{ background: '#202c33', color: '#fff', border: 'none', width: 34, height: 34, borderRadius: 10 }}
            >
              ✕
            </button>
          </div>
          ...
          <div style={{
            ...
            {[
              ...
              <button
                ...
                style={{
                  padding: 11, border: 'none', borderRadius: 11, textAlign: 'left',
                  background: socialTab === tab ? '#00a884' : '#111b21',
            ...
            <div style={{
              marginTop: 'auto', padding: 10, borderRadius: 12, background: '#111b21',
              color: '#7f8c98', fontSize: 11, lineHeight: 1.5
            }}>
              {authUser
                ? 'Hesabın aktif. Profil ve arkadaşlıkların bu cihazdaki oturumunda tutulur.'
                : 'Misafir olarak sohbet edebilirsin. Arkadaşlık ve profil için ücretsiz hesap aç.'}
            </div>
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
            ...
            {socialTab === 'global' && (
              <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
                ...
                <form
                  onSubmit={sendGlobalMessage}
                  style={{ padding: 10, borderTop: '1px solid #25313a', display: 'flex', gap: 7, background: '#111b21' }}
                >
                  <input
                    value={globalChatInput}
                    onChange={(e) => setGlobalChatInput(e.target.value)}
                    placeholder="Global sohbete bir şey yaz..."
                    style={{ ...styles.input, flex: 1 }}
                  />
                  <button
                    type="submit"
                    style={{ ...styles.buttonPrimary, padding: '10px 14px' }}
                  >
                    ➤
                  </button>
                </form>
                    ...
                    <div style={{ marginTop: 16 }}>
                      {friendSearchResults.map(u => (
                        <div key={u.username} style={{
                          ...
                          gap: 10, padding: 10, background: '#111b21', borderRadius: 12, marginBottom: 7
                        }}>
                          <div>
                            <div style={{ color: '#fff', fontWeight: 900 }}>{u.avatar} {u.username}</div>
                            <div style={{ color: '#7f8c98', fontSize: 11 }}>{u.status || u.bio || 'Henüz bir durum yazılmadı.'}</div>
                          </div>
                          <button
                            type="button"
                    ...
                    {friendRequests.length > 0 && (
                      <div>
                        ...
                        {friendRequests.map(r => (
                          <div key={r.id} style={{
                            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                            padding: 10, background: '#111b21', borderRadius: 12, marginBottom: 7
                          }}>
                            <span style={{ color: '#fff', fontWeight: 800 }}>{r.avatar || '🐱'} {r.fromUsername}</span>
                            <div style={{ display: 'flex', gap: 6 }}>
                              <button
                                type="button"
                                onClick={() => respondFriendRequest(r.id, 'accept')}
                                style={{ ...styles.buttonPrimary, padding: '7px 10px', fontSize: 11 }}
                              >
                                Kabul
                              </button>
                              <button
                                type="button"
                                onClick={() => respondFriendRequest(r.id, 'reject')}
                                style={{ background: '#202c33', color: '#fff', border: '1px solid #2d3b44', padding: '7px 10px', borderRadius: 10, fontWeight: 800 }}
                              >
                                Sil
                              </button>
                            </div>
                      ...
                      <div>
                        ...
                        {friends.map(f => (
                          <div key={f.username} style={{
                            ...
                            background: '#111b21', borderRadius: 12, marginBottom: 7
                          }}>
                            <span style={{ color: '#fff', fontWeight: 800 }}>{f.avatar} {f.username}</span>
                            <span style={{ color: '#53e6bc', fontSize: 11 }}>{f.status || 'Çevrimiçi'}</span>
                          </div>
                        ))}
                      </div>
                    )}
                ...
                {!authUser ? (
                  ...
                  <div style={{ maxWidth: 520 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
                      <div style={{
                        fontSize: 44, width: 66, height: 66, borderRadius: 18,
                        display: 'grid', placeItems: 'center', background: '#111b21',
                    ...
                    <select
                      value={myAvatar}
                      onChange={(e) => { setMyAvatar(e.target.value); localStorage.setItem('cm_user_avatar', e.target.value); }}
