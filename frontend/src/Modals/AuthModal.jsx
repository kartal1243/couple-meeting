...
export default function AuthModal({
  ...
  setShowAuthModal, errorMessage, setErrorMessage, styles
}) {
  const [errors, setErrors] = useState({});
  const [step, setStep] = useState(1); // 1: email, 2: password, 3: extra (register only)
    ...
    <div style={{
      ...
      <div style={{
        ...
        <button type="button" onClick={() => setShowAuthModal(false)}
          style={{ position: 'absolute', top: 14, right: 14, background: 'rgba(255,255,255,.06)', border: 'none', color: '#7f8c98', width: 32, height: 32, borderRadius: 10, cursor: 'pointer', fontSize: 13, display: 'grid', placeItems: 'center' }}>
          ✕
        </button>
        ...
        <div style={{ display: 'inline-flex', gap: 4, background: '#111b21', padding: 4, borderRadius: 10, marginBottom: 18 }}>
          <button type="button" onClick={() => { setAuthMode('login'); setStep(1); setErrors({}); }}
            style={{
              padding: '7px 14px', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 900, cursor: 'pointer',
              background: authMode === 'login' ? '#00a884' : 'transparent', color: authMode === 'login' ? '#fff' : '#7f8c98'
            }}>
            Giriş Yap
          </button>
          <button type="button" onClick={() => { setAuthMode('register'); setStep(1); setErrors({}); }}
            style={{
              padding: '7px 14px', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 900, cursor: 'pointer',
              background: authMode === 'register' ? '#00a884' : 'transparent', color: authMode === 'register' ? '#fff' : '#7f8c98'
            }}>
            Kayıt Ol
          </button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 20, justifyContent: 'center' }}>
          {Array.from({ length: totalSteps }).map((_, i) => (
            <div key={i} style={{
              ...
              background: i + 1 <= step ? '#00a884' : '#1a2634',
        ...
        {/* Error */}
        {errorMessage && (
          <div style={{ background: 'rgba(234,0,56,.12)', border: '1px solid rgba(234,0,56,.3)', color: '#ff6b81', padding: '10px 14px', borderRadius: 12, fontWeight: 800, fontSize: 12, marginBottom: 14, textAlign: 'center' }}>
            {errorMessage}
          </div>
        )}
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            {step > 1 && (
              <button type="button" onClick={prevStep}
                style={{ flex: 1, padding: '13px', borderRadius: 14, border: '2px solid #25313a', background: 'transparent', color: '#7f8c98', fontWeight: 900, fontSize: 14, cursor: 'pointer', transition: 'all 0.2s' }}>
                ← Geri
              </button>
            )}
            <button type="submit" disabled={authBusy}
              style={{
                ...
                background: authBusy ? '#1a2634' : 'linear-gradient(135deg, #00a884, #008f6f)',
                color: '#fff', fontWeight: 900, fontSize: 14, cursor: authBusy ? 'not-allowed' : 'pointer',
                opacity: authBusy ? 0.6 : 1, boxShadow: authBusy ? 'none' : '0 6px 20px rgba(0,168,132,0.3)',
                transition: 'all 0.2s'
              }}>
              {authBusy ? '⏳ İşleniyor...' : (
                (authMode === 'register' && step === 3) ? '✨ Hesabı Oluştur' :
                (authMode === 'login' && step === 2) ? '🚀 Giriş Yap' : 'Devam →'
              )}
            </button>
          </div>
        </form>
